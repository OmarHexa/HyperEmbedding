...
class Discriminator3(nn.Module):
    def __init__(self, c1=256, c2=128, c3=64) -> None:
        ...
        self.disc1 =nn.Sequential(
                        nn.Linear(c1 * 20 * 20, 100),
                        nn.BatchNorm1d(100),
                        nn.ReLU(True),
                        nn.Linear(100,2),
                        nn.LogSoftmax(dim=-1))
        # self.disc2 =nn.Sequential(
        #                 nn.Linear(c2 * 20 * 20, 100),
        #                 nn.BatchNorm1d(100),
        #                 nn.ReLU(True),
        #                 nn.Linear(100,2),
        #                 nn.LogSoftmax(dim=-1))
        self.disc3 =nn.Sequential(
                        nn.Linear(c3 * 20 * 20, 100),
                        nn.BatchNorm1d(100),
                        nn.ReLU(True),
                        nn.Linear(100,2),
                        nn.LogSoftmax(dim=-1))
    ...
    def forward(self,x1,x2,x3,alpha):
        ...
        x1 = self.disc1(x1.view(-1,c1*20*20))
        # x2 = self.disc2(x2.view(-1,c2*20*20))
        x3 = self.disc3(x3.view(-1,c3*20*20))
        # Combine log probabilities using logsumexp
        logits = torch.logsumexp(torch.stack([x1, x3]), dim=0)
        return logits
        # return x2
