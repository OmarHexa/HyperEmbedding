...
def trainUDA(model,discriminator,optimizer,criterion,domain_loss, source_loader,target_loader,device,epoch,n_epoch):
    ...
    for i, sample_src in enumerate(tqdm(source_loader)):
        ...
        loss_meter.update(src_loss.item())
...
def valUDA(args,model,criterion, source_loader,target_loader,visualizer,device,epoch):
    ...
    with torch.no_grad():
        for i, sample_src in enumerate(tqdm(source_loader)):
            ...
            emb_loss = criterion(output_s, s_inst, s_label,iou=True, iou_meter=iou_meter_src)
            src_loss = emb_loss.mean()
            ...
            emb_loss = criterion(output_t,t_inst,t_label,iou=True, iou_meter=iou_meter_target)
        
            target_loss = emb_loss.mean()
        ...
        if args['save']:
            ...
            t_offset,t_sigma,t_pred = visualizer.prepare_internal(output=output_t[0].cpu())
            ...
            s_offset,s_sigma,s_pred = visualizer.prepare_internal(output=output_s[0].cpu())
            ...
            grid = make_grid([s_gt,t_gt,s_pred,t_pred,s_offset,t_offset],nrow=3)
...
# set optimizer
    ...
    # Logger
    logger = Logger(('train', "aux", "val", 'target', 'val_iou','tar_iou'), 'loss')
    ...
    for epoch in range(start_epoch, args['n_epochs']):
        ...
        print('Starting epoch {}'.format(epoch))
        train_loss,train_auxloss = trainUDA(model,discriminator,optimizer,criterion,
                                                    domain_loss,src_train_dataloader,target_train_dataloader,
                                                    device,epoch, args['n_epochs'])
        val_loss,tar_loss,val_iou,tar_iou = valUDA(args,model,criterion,src_val_dataloader,target_train_dataloader,
